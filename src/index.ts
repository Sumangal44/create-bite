...
${colors.dim('Create a new bite project in JavaScript or TypeScript.')}
${colors.dim('When running in TTY, the CLI will start in interactive mode.')}
...
const FRAMEWORKS: Framework[] = [
  ...
  },
  {
    ...
    variants: [
      {
        name: 'create-bite-extra',
        display: 'Extra Bite Starters ↗',
        color: reset,
        customCommand: 'npm create bite-extra@latest TARGET_DIR',
      },
      {
        name: 'create-electron-bite',
        display: 'Electron ↗',
        color: reset,
        customCommand: 'npm create electron-bite@latest TARGET_DIR',
      },
    ],
  },
]
...
const defaultTargetDir = 'bite-project'

function run([command, ...args]: string[], options?: SpawnOptions) {
  const { status, error } = spawn.sync(command, args, options)
  if (status != null && status > 0) {
    prompts.log.error(colors.red(`✖ ${command} ${args.join(' ')} failed with status ${status}`))
    process.exit(status)
  }

  if (error) {
    prompts.log.error(colors.red(`\n✖ ${command} ${args.join(' ')} error!`))
    console.error(error)
    process.exit(1)
  }
}
...
function install(root: string, agent: string) {
  if (process.env._VITE_TEST_CLI) {
    prompts.log.step(colors.yellow(`📦 Installing dependencies with ${agent}... (skipped in test)`))
    return
  }
  prompts.log.step(colors.yellow(`📦 Installing dependencies with ${agent}...`))
...
function start(root: string, agent: string) {
  if (process.env._VITE_TEST_CLI) {
    prompts.log.step(colors.cyan('🚀 Starting dev server... (skipped in test)'))
    return
  }
  prompts.log.step(colors.cyan('🚀 Starting dev server...'))
...
async function init() {
  ...
  prompts.log.step(cyan(`🛠️  Scaffolding project in ${root}...`))
  ...
  if (immediate) {
    ...
    doneMessage += `${colors.green(colors.bold('✅ Done. Now run:'))}\n`
    if (root !== cwd) {
      doneMessage += `\n  cd ${
